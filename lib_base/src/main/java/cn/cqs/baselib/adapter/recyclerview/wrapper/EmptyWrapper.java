package cn.cqs.baselib.adapter.recyclerview.wrapper;
...
import cn.cqs.baselib.adapter.recyclerview.utils.WrapperUtils;
...
public class EmptyWrapper<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
   ...
   /* public void add(T t){
        if (mInnerAdapter instanceof CommonAdapter){
            ((CommonAdapter) mInnerAdapter).add(t);
        }
    }
    public void addAll(List<T> list){
        if (mInnerAdapter instanceof CommonAdapter){
            ((CommonAdapter) mInnerAdapter).addAll(list);
            notifyDataSetChanged();
        }
    }
    public void remove(int position){
        if (mInnerAdapter instanceof CommonAdapter){
            ((CommonAdapter) mInnerAdapter).remove(position);
        }
    }
    public void removeAll(int position,List<T> list){
        if (mInnerAdapter instanceof CommonAdapter){
            ((CommonAdapter) mInnerAdapter).removeAll(position,list);
        }
    }
    public void update(List<T> list){
        if (mInnerAdapter instanceof CommonAdapter){
            ((CommonAdapter) mInnerAdapter).update(list);
            notifyDataSetChanged();
        }
    }
    public void clear(){
        if (mInnerAdapter instanceof CommonAdapter){
            ((CommonAdapter) mInnerAdapter).clear();
        }
    }*/
}
