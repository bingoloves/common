package cn.cqs.baselib.skin.loader;
...
import cn.cqs.baselib.skin.config.SkinConfig;
import cn.cqs.baselib.skin.entity.SkinInfo;
import cn.cqs.baselib.skin.listener.ILoaderListener;
import cn.cqs.baselib.skin.listener.ISkinLoader;
...
//		LogUtils.e( "convertToColorStateList");
		...
		String resName = context.getResources().getResourceEntryName(resId);
//		LogUtils.e("resName = " + resName);
		if (isExtendSkin) {
//			LogUtils.e("isExtendSkin");
			int trueResId = mResources.getIdentifier(resName, "color", skinPackageName);
//			LogUtils.e("trueResId = " + trueResId);
...
//					LogUtils.e("trueColorList = " + trueColorList);
...
//					LogUtils.w("resName = " + resName + " NotFoundException :" + e.getMessage());
     ...
     * @param skinName
     */
	public SkinInfo getSkinInfo(String skinName){
		String dexPath = context.getFileStreamPath(skinName).getPath();
		PackageInfo packageInfo = context.getPackageManager().getPackageArchiveInfo(dexPath, PackageManager.GET_ACTIVITIES);
		if (packageInfo != null){
            ApplicationInfo appInfo = packageInfo.applicationInfo;
            String packageName = appInfo.packageName; //得到安装包名称
            String versionName = packageInfo.versionName; //得到版本信息
            int versionCode = packageInfo.versionCode;//得到版本号
            //Drawable icon = pm.getApplicationIcon(appInfo);//得到图标信息
            SkinInfo skinInfo = new SkinInfo();
            skinInfo.setName(skinName.substring(0,skinName.lastIndexOf(".")));
            skinInfo.setPackageName(packageName);
            skinInfo.setVersionCode(versionCode);
            skinInfo.setVersionName(versionName);
            skinInfo.setSkinName(skinName);
            skinInfo.setFile(context.getFileStreamPath(skinName));
